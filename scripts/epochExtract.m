...
function [zfSig, gci,p,winLength,vad]=epochExtract(wav,fs)
...
% winLength
%winLength=9;
% Derive the zero-frequency filtered signal...
...
%     p=p.*vad; gci=find(p==1); %uncomment if u want instants in voiced regions only
% 	p=zeros(length(wav)-1,1); p(gci)=1; p=p; gci=find(p==1);

plotFlag=0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
gci=gci+3;
p=zeros(length(wav),1); p(gci)=1;
